...
# 3. Updated View for HTML Form (Fixes JSONDecodeError)
def chatbot_view(request):
    ...
    if request.method == 'POST':
        # Get message from standard form POST data
        user_message = request.POST.get('message', '')
        
        # Get the bot logic results
        bot_data = get_chatbot_response(user_message)
